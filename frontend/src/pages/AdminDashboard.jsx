    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <>
          ...
          <section className="grid gap-6 lg:grid-cols-2">
            ...
            <div className={`rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white'} p-5 border ${darkMode ? 'border-gray-700' : 'border-slate-200'}`}>
              ...
              {mentorLoad.length === 0 ? (
                <div className="text-center py-12">
                  <UserCheck className="w-12 h-12 mx-auto opacity-50 mb-3" />
                  <p className="text-sm opacity-75">No assignments found yet.</p>
                </div>
              ) : (
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
            ...
            <div className={`rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white'} p-5 border ${darkMode ? 'border-gray-700' : 'border-slate-200'}`}>
              ...
              {deptStats.length === 0 ? (
                <div className="text-center py-12">
                  <Users className="w-12 h-12 mx-auto opacity-50 mb-3" />
                  <p className="text-sm opacity-75">No student data available.</p>
                </div>
              ) : (
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
          ...
          {/* Data Tables */}
          <div className={`rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white'} p-5 border ${darkMode ? 'border-gray-700' : 'border-slate-200'}`}>
            <h3 className="text-lg font-bold mb-4">Detailed Analytics</h3>
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                <thead>
                  <tr className={`text-left border-b ${darkMode ? 'border-gray-700' : 'border-slate-200'}`}>
                    ...
                    <th className="py-3 pr-6">Trend</th>
                    <th className="py-3 pr-6">Insight</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className={`border-b ${darkMode ? 'border-gray-700' : 'border-slate-200'}`}>
                    <td className="py-3 pr-6 font-medium">Average Students per Mentor</td>
                    <td className="py-3 pr-6">
                      {mentorLoad.length > 0 
                        ? (mentorLoad.reduce((acc, curr) => acc + curr.student_count, 0) / mentorLoad.length).toFixed(1)
                        : '0'}
                    </td>
                    <td className="py-3 pr-6">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${darkMode ? 'bg-emerald-500/20 text-emerald-400' : 'bg-emerald-100 text-emerald-700'}`}>
                        +2.3%
                      </span>
                    </td>
                    <td className="py-3 pr-6 opacity-75">Balanced distribution</td>
                  </tr>
                  <tr className={`border-b ${darkMode ? 'border-gray-700' : 'border-slate-200'}`}>
                    <td className="py-3 pr-6 font-medium">Most Popular Department</td>
                    <td className="py-3 pr-6">
                      {deptStats.length > 0 
                        ? deptStats.reduce((max, dept) => dept.count > max.count ? dept : max).department
                        : 'N/A'}
                    </td>
                    <td className="py-3 pr-6">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${darkMode ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-700'}`}>
                        Stable
                      </span>
                    </td>
                    <td className="py-3 pr-6 opacity-75">Leading in enrollment</td>
                  </tr>
                  <tr>
                    <td className="py-3 pr-6 font-medium">Mentor Utilization</td>
                    <td className="py-3 pr-6">
                      {mentorLoad.length > 0 ? '85%' : '0%'}
                    </td>
                    <td className="py-3 pr-6">
                    ...
                    <td className="py-3 pr-6 opacity-75">Good resource allocation</td>
                  </tr>
                </tbody>
              </table>
            </div>
